...
_BUILTIN_DEFINITIONS: list[dict] = [
    ...
    },
    {
        ...
        "label": "Mail Center（动态多域邮箱池）",
        "description": "连接 Mail Center 中央邮箱池；自动使用当前及未来合规域名，支持轮询、随机、最少使用及指定域名。",
        "driver_type": "mail_center_pool_api",
        "default_auth_mode": "token",
        "enabled": True,
        "category": "selfhost",
        "auth_modes": [{"value": "token", "label": "Integration Token"}],
        "fields": [
            {"key": "mail_center_api_url", "label": "中央 API 地址", "placeholder": "https://mail-center.example.com", "category": "connection"},
            {"key": "mail_center_integration_token", "label": "Integration Token", "secret": True, "category": "auth"},
            {
                "key": "mail_center_domain_strategy",
                "label": "域名策略",
                "type": "select",
            ...
            {"key": "mail_center_domain", "label": "指定域名（可选）", "placeholder": "留空时按策略自动选择", "hint": "指定后覆盖域名策略，但仍受中央白名单/黑名单约束。", "category": "identity"},
            {"key": "mail_center_prefix", "label": "邮箱前缀（可选）", "placeholder": "留空时仅生成随机本地名", "hint": "填写 aar 时生成 aar.随机串@域名；不填时无前缀。", "category": "identity"},
        ],
    },
    {
        "provider_type": "mailbox",
