...
from application.tasks import (
    ...
    recover_orphaned_claimed_tasks,
)
...
class TaskRuntime:
    ...
    def _loop(self) -> None:
        while self._running:
            try:
                ...
                print("[TaskRuntime] dispatcher loop error; continuing")
                traceback.print_exc()
    ...
    def _dispatch_once(self) -> None:
        ...
        recovered = recover_orphaned_claimed_tasks(active_task_ids)
        if recovered:
            print(f"[TaskRuntime] recovered {recovered} orphaned claimed task(s)")
    ...
    def _run_task(self, task_id: str) -> None:
        try:
            ...
            print(f"[TaskRuntime] task {task_id} crashed: {exc}")
            traceback.print_exc()
            try:
                TaskLogger(task_id).finish(TASK_STATUS_FAILED, error=f"Task runtime crashed: {exc}")
            except Exception:
                traceback.print_exc()
        finally:
            with self._lock:
