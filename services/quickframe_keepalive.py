"""Background keepalive loop for QuickFrame accounts."""
...
class QuickFrameKeepaliveWorker:
    ...
    def start(self) -> None:
        with self._lock:
            if self._running:
                return
            self._running = True
            self._thread = threading.Thread(target=self._loop, daemon=True, name="quickframe-keepalive")
            self._thread.start()
            print("[QuickFrameKeepalive] 已启动")

    def stop(self) -> None:
        with self._lock:
            self._running = False
        print("[QuickFrameKeepalive] 停止中")

    def _loop(self) -> None:
        time.sleep(10)
        while self._is_running():
            config = self._config()
            if not _as_bool(config.get("quickframe_keepalive_enabled"), True):
                self._sleep(60)
                continue

            now = time.time()
            heartbeat_interval = _bounded_int(
                config.get("quickframe_heartbeat_interval_seconds"),
                30,
                86400,
    ...
    def _target_account_ids(self) -> list[int]:
        with Session(engine) as session:
            accounts = session.exec(select(AccountModel).where(AccountModel.platform == "quickframe")).all()
            ...
            for account in accounts:
                ...
                if lifecycle not in ACTIVE_LIFECYCLE_STATUSES:
                    continue
                if _as_bool(overview.get("quickframe_keepalive_disabled"), False):
                    continue
    ...
    def _run_for_accounts(self) -> None:
        ...
        for account_id in self._target_account_ids():
            ...
            try:
                ...
                result = runtime.execute_action(command, log_fn=lambda message: print(f"[QuickFrameKeepalive] {message}"))
                if not getattr(result, "ok", False):
                    print(f"[QuickFrameKeepalive] account {account_id} failed: {getattr(result, 'error', '')}")
            except Exception as exc:
                print(f"[QuickFrameKeepalive] account {account_id} failed: {exc}")
            finally:
...
quickframe_keepalive_worker = QuickFrameKeepaliveWorker()
