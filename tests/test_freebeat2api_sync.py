...
def test_build_freebeat2api_payload_can_enable_remote_maintenance():
    account = Account(
        platform="freebeat",
        email="user@example.com",
        password="",
        extra={
            ...
            "freebeat2api_enable_auto_maintenance": "true",
        },
    )
...
def test_sync_account_to_freebeat2api_posts_and_refreshes_balance_and_sign_in():
    ...
    with patch("core.freebeat2api_sync._get_freebeat2api_config", return_value=("http://localhost:8788", "key", 1)):
...
def test_sync_account_to_freebeat2api_skips_when_unconfigured():
    with patch("core.freebeat2api_sync._get_freebeat2api_config", return_value=("", "", 1)):
