...
def _auto_sync_higg2api(task_logger: TaskLogger, account) -> None:
    if getattr(account, "platform", "") != "higg":
        return
    try:
        from core.higg2api_sync import is_higg2api_configured, sync_account_to_higg2api

        result = sync_account_to_higg2api(account, log_fn=task_logger.log)
        if result:
            task_logger.log("  [Higg2API] Higgsfield account synced")
        elif is_higg2api_configured():
            task_logger.log(
                "  [Higg2API] auto sync skipped or failed; check higg2api_url/API key and previous warning logs",
                level="warning",
            )
    except Exception as exc:
        task_logger.log(f"  [Higg2API] auto sync error: {exc}", level="warning")
...
def _merge_lingya_followup_data(account, data: dict[str, Any]) -> None:
    ...
    compact_keys = {
        ...
        "last_publish_initial_first_post_credit_granted",
        "last_publish_first_post_credit_granted",
    ...
    try:
        ...
        result = platform.execute_action(
            ...
            account,
            {
                ...
                "highlight_fallback_delay": _task_config_value(
                    extra_cfg,
                    "lingya_qq_publish_highlight_fallback_delay",
                    30,
                ),
                "credit_timeout": _task_config_value(extra_cfg, "lingya_qq_publish_credit_timeout", 1800),
                "credit_poll_interval": _task_config_value(extra_cfg, "lingya_qq_publish_credit_poll_interval", 30),
...
def _execute_register_task(payload: dict[str, Any], logger: TaskLogger) -> None:
    ...
    def _do_one(index: int) -> bool | str:
        ...
        try:
            ...
            _auto_sync_lingya2api(logger, account)
            _auto_sync_higg2api(logger, account)
